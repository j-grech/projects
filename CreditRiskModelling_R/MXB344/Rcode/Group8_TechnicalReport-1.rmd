...
A variable that holds more than 30% unique values is not ideal to be used for modelling as its too variable.
...
#### Downsampling
```{r rfdown, warning=FALSE, message=FALSE, echo=FALSE}
##down sampling
set.seed(420)
col_idx <- grep("repay_fail", names(train.rf))
train.rf <- train.rf[, c((1:ncol(train.rf))[-col_idx],col_idx)]
...
```{r lasso, warning=FALSE, message=FALSE, echo=FALSE}
